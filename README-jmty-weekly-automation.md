...
## 実行の考え方
Drive への画像アップロードとスプレッドシート反映は、GWS が動く場合は GWS 経由で行います。GWS 認証切れなどで失敗した場合だけ、Chrome で Google Drive を開いて行います。
...
- Discord 通知用に `TEAM_INFO_DISCORD_JMTY_WEBHOOK_URL` か `config/discord-jmty-webhook.json` を設定する
...
## Google Drive 反映
まず `sync-drive --purge-account-images` で、GWS 経由の Drive / シート反映を試します。
...
## Discord 通知
GWS 認証切れ、権限エラー、APIエラーなどで GWS 反映が失敗した場合は、Discord デスクトップアプリで次の場所へ通知します。
...
Discord 通知は、環境変数 `TEAM_INFO_DISCORD_JMTY_WEBHOOK_URL` または `config/discord-jmty-webhook.json` の `url` を使います。
