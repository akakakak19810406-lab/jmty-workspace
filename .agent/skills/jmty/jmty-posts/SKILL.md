...
| Boost.Work と でぐちの両方で同時投稿する | `dual-account-posting` |

## 共通ルール
- 投稿文作成、再作成、リライトでは `.agent/skills/jmty/no-python-post-writing/SKILL.md` と `.agent/skills/jmty/jmty-post-rewrite-rules/SKILL.md` を必ず読む。
- Boost.Work と でぐちの両方へ投稿する場合、または会社名の出し分けが必要な場合は `.agent/skills/jmty/jmty-dual-account-posting/SKILL.md` を必ず読む。
...
### 工場投稿の固定方針
- 工場投稿は「実案件を紹介している感じ」を最優先する。仕事の内容、勤務条件、寮、休日、社会保険、応募導線などの厚みを削って、薄い汎用求人文にしない。
- 工場投稿の基本訴求は `月収40万円` と `入社祝い金160万円`。タイトル、冒頭、条件欄のいずれかで両方を目立たせる。
- 好条件を出す場合も、別案件の条件を1案件の確定条件に見せすぎない。必要に応じて投稿文側にだけ `※金額・特典は案件や規定により異なります。` を軽く入れる。
- 全員分の工場投稿を同じ構成・同じ口調に寄せない。短文強訴求、相談型、未経験向け、寮付き訴求、急募感、生活改善系、淡々とした条件提示など、スタイルを分散させる。

### 工場画像プロンプトの固定方針
- 工場画像は `月収40万円` と `入社祝い金160万円` の両方を主役にする。月収だけを大きくして、祝い金を弱くしない。
- 画像内には `規定あり`、`案件により異なります` などの注意書きを入れない。注意書きは投稿文側だけで扱う。
- 工場背景、作業服、ヘルメット、機械、人物を必須にしない。画風プロンプト、数字訴求、広告映えするレイアウトを優先する。
- 人物・キャラクターを使う場合、性別不明の抽象キャラクター主体の画風を除き、前面の人物は基本的に女性スタッフまたは女性キャラクターにする。作業服や工場感に寄せすぎず、清潔感と親しみやすさを優先する。
...
週次一括では、最初にスプレッドシート上の地域と投稿文担当をランダム割当し、工場は H列の地域を正本にして投稿文をAI再作成し、その投稿文を元に画像を作る。
...
## dual-account-posting: 両アカウント同時投稿
Boost.Work と でぐちの両方へ同じ投稿サイクルで投稿する場合に使う。

1. `.agent/skills/jmty/jmty-auto-posting-rules/SKILL.md` を読む。
2. `.agent/skills/jmty/jmty-dual-account-posting/SKILL.md` を読む。
3. 各アカウントの会社名、投稿数、地域履歴、確認中ステータス、次回投稿可能時刻を別々に管理する。
4. Boost.Work は会社名 `Boost.Work`、でぐちは会社名 `株式会社Keystone` を使用する。
5. 片方が確認中・認証・CAPTCHA などで止まっても、もう片方が投稿可能ならそのアカウントだけ続行する。
