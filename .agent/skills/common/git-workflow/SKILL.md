...
**【手順E】PR がマージされたら `main` に戻す:**
- `other` の作業ブランチで作った PR がマージ済みになったら、作業ブランチに留まらず `main` へ切り替える。
- 切り替え前に `git -C "$JMTY_ROOT" status --porcelain` で未コミット変更がないか確認する。
- 未コミット変更がある場合は、勝手に捨てず、ユーザーへ扱いを確認する。
- `origin/main` を取り直してから `main` に戻り、通常の作業場所を最新にする。

```bash
git -C "$JMTY_ROOT" fetch origin --prune
git -C "$JMTY_ROOT" switch main || git -C "$JMTY_ROOT" switch -c main --track origin/main
git -C "$JMTY_ROOT" pull --ff-only origin main
git -C "$JMTY_ROOT" status --short --branch
```
