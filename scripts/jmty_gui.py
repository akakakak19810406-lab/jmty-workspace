...
def image_region_instruction(kind: str, region: str, stale_regions: list[str]) -> str:
    ...
    target = str(region or "").strip() or "地域未設定"
    if normalize_kind(kind) == "factory":
        usage = f"画像内の勤務地・エリア表記は「{target}」だけにする。工場求人の勤務地として扱う。"
    else:
        usage = f"画像内の対象地域表記は「{target}から応募OK」または「{target}募集」だけにする。勤務地は「完全在宅」として扱う。"
    stale = ""
    if stale_regions:
